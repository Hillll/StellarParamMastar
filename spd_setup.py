...
class spd_setup:
    ...
    def __init__(self):
        ...
        self.nums_file = 'nums_todo.npz'   # ndarray of indices to do from self.spec_file

        # input options
        self.alpha = True
        self.min_lambda = -999  # set wavelength lower limit. If -999 then minimum lambda is used.
        self.max_lambda = -999  # set wavelength upper limit. If -999 then maximum lambda is used.
        ...
        # mcmc params
        self.early_stopping = False   # whether to measure convergence and stop when deemed comverged
        if self.alpha:
            self.ndim = 4
        else:
            self.ndim = 3
        self.nwalkers = 10  # N MCMC walkers
